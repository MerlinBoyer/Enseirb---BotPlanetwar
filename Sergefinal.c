...
#define TAILLE_MAX 100   /* les constantes sont arbitraires et prevues
                            assez large pour que la taille des tableaux ne soit jamais insuffisante*/
...
struct planet {
    ...
    int ships;
    int growth;
    int id;     /*chaque planete ou flotte possede une i.d unique donnee par le moteur
                de jeu et qui permet de les identifier.   */
...
/*****      La structure universe regroupe les autres structures ; 
            cela permet de placer un seul element en parametre des 
            fonctions pour acceder a toutes les donnees du tour.
*****/
struct universe {
...
/***    ajoute une flotte a la liste des flottes (par exemple apres un 
        ordre d'attaque pour actualiser les flottes alliees sans attendre 
        le prochain tour)
***/
void add_fleet(struct universe* universe , struct indice* indice, struct fleet* one_more_fleet){
    copy_fleet( one_more_fleet, &(universe->f_allied[(indice->indf_ally)]));
            (indice->indf_ally)++;
}
...
void game_stat(char buffer[], struct universe* universe, int* id_planet, int* id_fleet, struct indice* indice){
    ...
    if(buffer[0] == 'P'){
        ...
        if(current_planet.owner == 1){
            copy_planet(&current_planet, &(universe->p_allied[(indice->indp_ally)]));
        ...
        if(current_planet.owner == 2){
            copy_planet(&current_planet, &(universe->p_ennemy[(indice->indp_ennemy)]));
            (indice->indp_ennemy)++;
            }
        if(current_planet.owner == 0){
            copy_planet(&current_planet, &(universe->p_neutral[(indice->indp_neutral)]));
            (indice->indp_neutral)++;
            }
    ...
    if(buffer[0] == 'F'){
        ...
        if(current_fleet.owner == 1){
            copy_fleet(&current_fleet, &(universe->f_allied[(indice->indf_ally)]));
            (indice->indf_ally)++;
            }
        if(current_fleet.owner == 2){
            copy_fleet(&current_fleet, &(universe->f_ennemy[(indice->indf_ennemy)]));
            (indice->indf_ennemy)++;
            }
...
void attack(struct universe* universe , struct indice* indice , struct planet* planet_source, struct planet* planet_destination, int nb_ships){
    if(planet_source->id != planet_destination->id && nb_ships < planet_source->ships){
        ...
        struct fleet one_more_fleet = {1, nb_ships ,planet_source->id, planet_destination->id, 1, distance( *planet_source,*planet_destination)+1, indice->indf_ally};
        /*      l'initialisation renvoie des erreurs car les parametres de la stucture ne
                sont pas evaluables au moment de la compilation or la fonction attack rempli
                parfaitement son role.      */
        add_fleet(universe ,indice, &(one_more_fleet)); /*  ajoute la flotte lancee au tableau des flottes pour actualiser
                                                            les flottes en cours sans passer par le moteur de jeu (avant le tour suivant).  */
    }
}
...
/*  Programe qui classe les planetes de la plus proche a la plus eloignee
...
*****/   
int already_gone(struct fleet fleets[NB_ARMIES], int position, struct indice* indice){
    int i;
    for( i=0 ; i< (indice->indf_ally) ; i++){
        if(fleets[i].destination == position){
            return 1;
        }
    }
    return 0;
...
int already_taken(struct fleet fleet[NB_ARMIES], struct planet planet, struct indice* indice, struct planet planetes[MAX_PLANET]){
    ...
    if ( nb_ships_incoming > planet.ships+((planet.growth)*10)){
        return 1;
    }
    return 0;
}
...
int planet_safe(struct planet* planet, struct fleet fleet[NB_ARMIES], struct indice* indice , int nb_attacking){
    ...
    for( i=0 ; i<(indice->indf_ennemy) ; i++){
        if(fleet[i].destination == planet->id){
            nb_ships_incoming += fleet[i].ships;
            if( (planet->ships)+(planet->growth * fleet[i].remaining_turns)-(nb_attacking) < nb_ships_incoming+1 ){
                return 0;
            }
        }
    }
...
/****   envoie une flotte suffisante pour conquerir une planete ennemies si la flotte
        sur la planete de depart est suffisante (repaire les planetes ennemies faibles
         et proches).
****/
int easy_take(struct universe* universe, struct planet planet, struct planet p_cibles[MAX_PLANET], struct indice* indice){
    ...
    if( planet_safe( &(planet), universe->f_ennemy, indice, nb_ships_recquired ) && !already_gone(universe->f_allied, ennemy_sorted[0].id, indice) && nb_ships_recquired < planet.ships){
        attack( universe, indice,  &(planet), &(ennemy_sorted[0]), nb_ships_recquired);
        return 1;
    }
    return 0;
}
...
/****      la planete attaque la planete neutre la plus proche
        ...
        deja en chemin.
****/
void colonize_Close(struct universe* universe, struct planet p_allied, struct indice* indice){
        struct planet neutral_sorted[MAX_PLANET];
        close_sorting( universe->p_neutral , neutral_sorted , indice->indp_neutral , p_allied);
        if( ( planet_safe(&(p_allied), universe->f_ennemy, indice , neutral_sorted[0].ships+10)) && !already_gone(universe->f_allied, neutral_sorted[0].id, indice) && p_allied.ships -(neutral_sorted[0].ships+10) >= 0){
                    attack( universe, indice,  &(p_allied) , &(neutral_sorted[0]) , neutral_sorted[0].ships+10);
        }
}
...
/****      la planete attaque la planete neutre de la moins peuplee 
        ...
        deja en chemin.
****/
void colonize_Weak(struct universe* universe, struct planet p_allied, struct indice* indice){
    struct planet neutral_sorted[MAX_PLANET];
    weak_sorting( universe->p_neutral , neutral_sorted , indice->indp_neutral);
    int i;
    int j;
    for( i=0; i<=(indice->indp_ally) ; i++){
        for( j=0 ; j<(indice->indp_neutral) ; j++){
            struct planet close_ennemy[MAX_PLANET];
            close_sorting( universe->p_ennemy , close_ennemy , indice->indp_ennemy , neutral_sorted[j]);
            if( planet_safe(&(p_allied), universe->f_ennemy, indice, neutral_sorted[j].ships+15) && !already_gone(universe->f_allied, neutral_sorted[j].id, indice)){
                if( (distance(p_allied , neutral_sorted[j]) < 13 || p_allied.ships-(neutral_sorted[j].ships+15) > 50) && distance(close_ennemy[0] , neutral_sorted[j]) > 6){
                    attack( universe, indice,  &(p_allied) , &(neutral_sorted[j]) , neutral_sorted[j].ships+15);
                    return;
                }
            }
        }
...
/*****      toutes les planetes alliees (si elles ne risquent pas de se faire conquerir par l'ennemi),
        envoient la moitié de leur flotte disponible vers une planete ennemie.
*****/


void attack_all(struct universe* universe, struct indice* indice){
    int i=0;
    int j=0;
    struct planet ennemy_sorted[MAX_PLANET];
    while(i < (indice->indp_ally)) {
        close_sorting( universe->p_ennemy , ennemy_sorted , indice->indp_ennemy , universe->p_allied[i]);
        if( universe->p_allied[i].ships >= 60 && planet_safe( &(universe->p_allied[i]), universe->f_ennemy, indice, (universe->p_allied[i].ships)/2) && j < (indice->indp_ennemy)){
                while( already_taken( universe->f_allied, ennemy_sorted[j], indice, universe->p_allied)){
                    j++;
                    if( j == indice->indp_ennemy){return;}
                }
                attack( universe, indice, &(universe->p_allied[i]) , &(ennemy_sorted[j]) , (universe->p_allied[i].ships)/2);
...
void serge( struct universe* universe, struct indice* indice){
    int i;
    if(((indice->indp_neutral) > (indice->indp_ally) ||  (indice->indp_ennemy)>(indice->indp_ally)) && (indice->indp_neutral)*(indice->indp_ennemy) > 0){
                /* on verifie aussi qu'il reste des planetes neutres ou ennemies sinon le bot crash  */
        
        for(i=0 ; i<(indice->indp_ally) ; i+=2){
            if (!easy_take( universe, universe->p_allied[i], universe->p_ennemy, indice)){
                colonize_Weak(universe, universe->p_allied[i], indice);
            }
        }
        for(i=1 ; i< (indice->indp_ally) ; i += 2){
            if (!easy_take( universe, universe->p_allied[i], universe->p_ennemy, indice)){
                colonize_Close(universe , universe->p_allied[i], indice);
            }
        }
    }
    else{
        if( indice->indp_ennemy > 0){
            attack_all(universe , indice);
        }
    }
}
...
int main(int argc, char *argv[]){
    ...
    while (!feof(stdin)){
        ...
        if (buffer[0]=='g' && buffer[1]=='o'){
            ...
            game_stat(buffer, &universe, &id_planet, &id_fleet, &indice);
        }
    }
    return 0;
}
