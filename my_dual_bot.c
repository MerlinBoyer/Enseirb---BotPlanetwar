  ...
  while(planetes_source != NULL)
    {
      ...
      planete_prospection = NULL;
      planetes_destination = carte->planetes_neutres;
...
void bot_attaque(struct carte * carte)
{
  ...
  struct planete * attaque_planete;
  while(planetes_source != NULL)
    {
      int tours_attaque_planete = 0;
      attaque_planete = NULL;
      vaisseaux_dispos = (planetes_source->infos_planete.nombre_vaisseaux) - 1;
      ...
      while(planetes_nemesis != NULL)
	{
	  ...
	  /*FILE * file = fopen("log2.txt", "a");
	  fprintf(file, "Nombre vaisseaux ennemis %d\n", vaisseaux_ennemies_arrivee);
	  fclose(file);*/
	  if(vaisseaux_ennemies_arrivee < vaisseaux_dispos)
	      ...
	      if(!tours_attaque_planete)
		{
		  ...
		  attaque_planete = planetes_nemesis;
		} else if(tours_distance < tours_attaque_planete){
		tours_attaque_planete = tours_distance;
		attaque_planete = planetes_nemesis;
	      } else if(tours_distance == tours_attaque_planete){
		if(planetes_nemesis->infos_planete.croissance > attaque_planete->infos_planete.croissance)
		  attaque_planete = planetes_nemesis;
	      }
      ...
      if(attaque_planete && verif_attaque(carte, attaque_planete))
	{
	  printf ("%d %d %d\n",
		  planetes_source->infos_planete.numero_planete,
		  attaque_planete->infos_planete.numero_planete,
		  attaque_planete->infos_planete.nombre_vaisseaux + tours_attaque_planete*(attaque_planete->infos_planete.croissance) +1);
	}
      planetes_source = planetes_source->planete_suivante;
    }
  go();
...
void dual_bot(struct carte *carte)
{
  ...
  if(croissance_totale_alliee > croissance_totale_ennemie + 5 || !(carte->planetes_neutres))
    {
      bot_attaque(carte);
    } else {
