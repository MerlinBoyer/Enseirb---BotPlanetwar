...
void colonize_allWeak(struct planet p_allied[MAX_PLANET], struct planet p_neutral[MAX_PLANET], struct indice* indice, struct fleet fleet_ally[NB_ARMIES]){
    ...
    for( i=0; i<=(indice->indp_ally) ; i++){
        for( j=0 ; j<(indice->indp_neutral) ; j++){
            if( (p_allied[i].ships)-(neutral_sorted[j].ships) > 101 || !already_gone(fleet_ally, neutral_sorted[j].id, indice)){
                    attack( &(p_allied[i]) , &(neutral_sorted[j]) , neutral_sorted[j].ships+1); 
            }
        }
...
void attack_all(struct planet p_allied[MAX_PLANET], struct planet p_ennemy[MAX_PLANET], struct indice* indice){
    ...
    for( i=0 ; i<(indice->indp_ally) ; i++){
        if(p_allied[i].ships>=100){
            attack(&(p_allied[i]) , &(p_ennemy[0]) , 10);
        }
    }
}


/* il faudrait coloniser qu'une fois*/




            
void serge(struct planet p_allied[MAX_PLANET], struct planet p_ennemy[MAX_PLANET], struct planet p_neutral[MAX_PLANET], struct indice* indice, struct fleet f_allied[NB_ARMIES]){
    if(indice->indp_neutral > indice->indp_ally){
        colonize_allWeak(p_allied, p_neutral, indice, f_allied);
    }
    else{
...
int main(int argc, char *argv[]){
    ...
    while (!feof(stdin)){
        ...
        if (buffer[0]=='g' && buffer[1]=='o'){
            serge(p_allied, p_ennemy, p_neutral, &indice, f_allied);
            init_indice(&indice);
            id_planet = 0;
            id_fleet = 0;
            go();           
