...
/***    ajoute une flotte a la liste des flottes (par exemple apres un 
        ...
        le prochain tour)
***/
void add_fleet(struct universe* universe , struct indice* indice){
    struct fleet one_more_fleet;
...
*****/
int float_incoming (struct planet planet_ref, struct universe* universe, struct indice* indice ){
    int i;
    int ships_incoming = 0;
    for( i=0 ; i<(indice->indf_ennemy) ; i++){
        if (universe->f_ennemy[i].destination == planet_ref.id){
            ships_incoming += universe->f_ennemy[i].ships;
        }
    }
...
void attack_all(struct universe* universe, struct indice* indice){
    ...
    while(i < (indice->indp_ally)) {
        ...
        if( universe->p_allied[i].ships >= 60 && planet_safe( &(universe->p_allied[i]), universe->f_ennemy, indice, (universe->p_allied[i].ships)/2) && j < (indice->indp_ennemy)){
                ...
                attack(&(universe->p_allied[i]) , &(ennemy_sorted[j]) , (universe->p_allied[i].ships)/2);
        }
        i++;
    }
}
...
void serge( struct universe* universe, struct indice* indice){
    int i;
    if(((indice->indp_neutral) > (indice->indp_ally)*2 ||  (indice->indp_ennemy)>(indice->indp_ally)) && (indice->indp_neutral) > 0){
